...
// File: gauge_finishdesign1_OTA_watchdog_heartbeat.ino
//
// Adds:
...
// - LED green 3mm heartbeat flashing on display every 1sec
//=====================================================================
...
const unsigned long HEARTBEAT_BLINK_MS = 500;

// Approximately 3 mm in diameter on a 240-pixel, 1.28-inch display.
// Positioned immediately above the digital pressure readout.
const int HEARTBEAT_X = 120;
const int HEARTBEAT_Y = 185;
const int HEARTBEAT_RADIUS = 11;
...
void updateHeartbeat() {
  if (millis() - lastHeartbeatToggle < HEARTBEAT_BLINK_MS) return;

  lastHeartbeatToggle = millis();
  heartbeatOn = !heartbeatOn;
  tft.fillCircle(HEARTBEAT_X, HEARTBEAT_Y, HEARTBEAT_RADIUS,
                 heartbeatOn ? GC9A01A_GREEN : GC9A01A_BLACK);
}
...
void setupOTA() {
  ...
  ArduinoOTA.onProgress([](unsigned int progress, unsigned int total) {
    // ArduinoOTA.handle() can remain busy longer than the normal loop's
    // watchdog interval. Service the watchdog as each OTA block arrives.
    esp_task_wdt_reset();
